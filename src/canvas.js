  ...
  function drawMenuShards(hub, ep) {
    ITEMS.forEach((label, i) => {
      ...
      drawPoly(hub, sh.pts, ep,
        ...
        isActive ? 'rgba(255, 255, 255, 0.9)' : 'rgba(70,170,255,.55)',
        isActive ? 2 : 1);
      ctx.globalAlpha = ep * (isActive ? .6 : .32);
      drawPoly(hub, sh.hlPts, ep,
        isActive ? 'rgba(212, 229, 242, 0.65)' : 'rgba(227, 246, 255, 0.55)', '', 0);
      ...
      if (ep > .4) {
        ...
        if (isActive) { ctx.shadowColor = 'rgba(255, 255, 255, 0.9)'; ctx.shadowBlur = 12; }
